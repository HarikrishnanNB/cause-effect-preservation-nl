...
for COUP_COEFF in COUP_COEFF1:
    ...
    ROW = ROW+1
    RESULT_PATH = PATH + '/LSTM_FEATURES/'  + DATA_NAME_TRAIN + '/' + str(COUP_COEFF) +'/'
    ...
    model = Sequential()
    layer_1 = model.add(LSTM(72, input_dim=input_dim, return_sequences=False, name="layer_1"))
    layer_2 = model.add(Dense(out_dim, activation='softmax', name="layer_2"))
    ...
    model.load_weights('LSTM/logs/'+ DATA_NAME_TRAIN +'/' +str(COUP_COEFF)+'/'+'check-points'+'/'+'checkpoint.hdf5')
    ...
    layer_output_class_0 = get_3rd_layer_output([class_0_data.reshape(class_0_data.shape[0], 1, class_0_data.shape[1])])[0]
    layer_output_class_1 = get_3rd_layer_output([class_1_data.reshape(class_1_data.shape[0], 1, class_1_data.shape[1])])[0]
    
        # Saving the Firing Time and Firing Rate
    io.savemat(RESULT_PATH + 'class_0_indep_lstm_features.mat', {'class_0_indep_lstm_features': layer_output_class_0 })
    io.savemat(RESULT_PATH + 'class_1_dep_lstm_features.mat', {'class_1_dep_lstm_features': layer_output_class_1})
