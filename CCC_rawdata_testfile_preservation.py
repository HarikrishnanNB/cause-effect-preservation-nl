...
for COUP_COEFF in COUP_COEFF1:
    ...
    for data_instance_no in range(0, TRIALS):
        ...
        ccc_M_S = CCC.compute(class_1_data[data_instance_no, :], class_0_data[data_instance_no, :], LEN_past=100, ADD_meas=15, STEP_size=50, n_partitions=4)
        ccc_S_M = CCC.compute(class_0_data[data_instance_no, :], class_1_data[data_instance_no, :], LEN_past=100, ADD_meas=15, STEP_size=50, n_partitions=4)
        M_S.append(ccc_M_S)
        S_M.append(ccc_S_M)
...
plt.plot(COUP_COEFF1, M_causes_S, '-*k', markersize = 10, label = "M causes S")
plt.plot(COUP_COEFF1, S_causes_M, '-or', markersize = 10, label = "S causes M")
...
plt.tight_layout()
plt.savefig(RESULT_PATH_FINAL+"/NL-"+DATA_NAME+"-raw_data-CCC-2.jpg", format='jpg', dpi=300)
plt.savefig(RESULT_PATH_FINAL+"/NL-"+DATA_NAME+"-raw_data-CCC.eps", format='eps', dpi=300)
plt.show()

# Saving Results

np.save(RESULT_PATH_FINAL +'NL_CCC_raw_data_M_causes_S.npy', M_causes_S)
np.save(RESULT_PATH_FINAL +'NL_CCC_raw_data_S_causes_M.npy', S_causes_M)