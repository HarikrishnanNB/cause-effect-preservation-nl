...
for COUP_COEFF in COUP_COEFF1:
    ...
    ROW = ROW+1
    RESULT_PATH = PATH + '/DL_FEATURES/'  + DATA_NAME_TRAIN + '/' + str(COUP_COEFF) +'/'
    ...
    model = Sequential()
    layer_1 = model.add(Dense(5000, input_dim=input_dim, activation='sigmoid', name="layer_1"))
    layer_2 = model.add(Dense(500, activation='relu', name="layer_2"))
    layer_3 = model.add(Dense(100, activation='relu', name="layer_3"))
    layer_4 = model.add(Dense(30, activation='relu', name="layer_4"))
    ...
    model.load_weights('logs/'+ DATA_NAME_TRAIN +'/' +str(COUP_COEFF)+'/'+'check-points'+'/'+'checkpoint.hdf5')
    ...
    layer_output_class_0 = get_3rd_layer_output([class_0_data])[0]
    layer_output_class_1 = get_3rd_layer_output([class_1_data])[0]
    
        # Saving the Firing Time and Firing Rate
    io.savemat(RESULT_PATH + 'class_0_indep_dl_features.mat', {'class_0_indep_dl_features': layer_output_class_0 })
    io.savemat(RESULT_PATH + 'class_1_dep_dl_features.mat', {'class_1_dep_dl_features': layer_output_class_1})
