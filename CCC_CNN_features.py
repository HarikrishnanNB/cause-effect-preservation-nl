...
for COUP_COEFF in COUP_COEFF1:
    ...
    RESULT_PATH = PATH + '/CNN_FEATURES/'  + DATA_NAME + '/' + str(COUP_COEFF) +'/'



    Y_independent_data = io.loadmat(RESULT_PATH + 'class_0_indep_cnn_features.mat')
    # Y_independent_label = io.loadmat(RESULT_PATH + 'Y_independent_label_class_0.mat')
    
    X_dependent_data = io.loadmat(RESULT_PATH + 'class_1_dep_cnn_features.mat' )
    # X_dependent_label = io.loadmat(RESULT_PATH + 'X_dependent_label_class_1.mat')
    
    class_0_data = Y_independent_data['class_0_indep_cnn_features'][0:VAR, 0:LEN_VAL]
    # class_0_label = Y_independent_label['class_0_indep_raw_data_label'][0:VAR, 0:LEN_VAL]
    class_1_data = X_dependent_data['class_1_dep_cnn_features'][0:VAR, 0:LEN_VAL]
    ...
    for data_instance_no in range(0, TRIALS):
        ...
        ccc_M_S = CCC.compute(class_1_data[data_instance_no, :], class_0_data[data_instance_no, :], LEN_past=20, ADD_meas=10, STEP_size=2, n_partitions=2)
        ccc_S_M = CCC.compute(class_0_data[data_instance_no, :], class_1_data[data_instance_no, :], LEN_past=20, ADD_meas=10, STEP_size=2, n_partitions=2)
        M_S.append(ccc_M_S)
        S_M.append(ccc_S_M)
...
plt.plot(COUP_COEFF1, M_causes_S, '-*k', markersize = 10, label = "M causes S (CNN)")
plt.plot(COUP_COEFF1, S_causes_M, '-or', markersize = 10, label = "S causes M (CNN)")
...
plt.tight_layout()
plt.savefig(RESULT_PATH_FINAL+"/CNN-"+DATA_NAME+"-raw_data-CCC-2.jpg", format='jpg', dpi=300)
plt.savefig(RESULT_PATH_FINAL+"/CNN-"+DATA_NAME+"-raw_data-CCC.eps", format='eps', dpi=300)
plt.show()

# Saving Results

np.save(RESULT_PATH_FINAL +'CNN_CCC_feature_data_M_causes_S.npy', M_causes_S)
np.save(RESULT_PATH_FINAL +'CNN_CCC_feature_data_S_causes_M.npy', S_causes_M)
