...
def dnn_2_layer(input_dim, out_dim, batch_size_val, epochs_val, RESULT_PATH, normalized_traindata , y_train):
    ...
    model = Sequential()
    model.add(LSTM(72, input_dim=input_dim, return_sequences=False))
...
for COUP_COEFF in COUP_COEFF1:
    ...
    PATH = os.getcwd()
    RESULT_PATH_LSTM = PATH + '/LSTM/logs/'  + DATA_NAME +'/'+str(COUP_COEFF)+'/' +'/check-points/' 

    X_train = np.reshape(traindata,(traindata.shape[0], 1, traindata.shape[1]))
    X_test = np.reshape(testdata, (testdata.shape[0], 1, testdata.shape[1]))
    ...
    model = dnn_2_layer(input_dim, out_dim, batch_size_val, epochs_val, RESULT_PATH_LSTM, X_train, train_label_)
    
    model.load_weights(RESULT_PATH_LSTM + "checkpoint.hdf5")
...
plt.plot(COUP_COEFF1,F1_score_Result_array, '-*k', markersize = 10, label = "LSTM F1-Score")
...
plt.tight_layout()
plt.savefig(RESULT_PATH_FINAL+"/LSTM-"+DATA_NAME+"-F1_score_vs_coup_coeff.jpg", format='jpg', dpi=300)
plt.savefig(RESULT_PATH_FINAL+"/LSTM-"+DATA_NAME+"-F1_score_vs_coup_coeff.eps", format='eps', dpi=300)
plt.show()
